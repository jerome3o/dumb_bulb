...
export const encrypt = (data: any, deviceKey: TapoDeviceKey) : string => {
    let cipher = crypto.createCipheriv(AES_CIPHER_ALGORITHM, deviceKey.key, deviceKey.iv);
...
export const decrypt = (data: string, deviceKey: TapoDeviceKey) : any => {
    let cipher = crypto.createDecipheriv(AES_CIPHER_ALGORITHM, deviceKey.key, deviceKey.iv);
    let ciphertext = cipher.update(Buffer.from(data, 'base64'));
